      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            ...
            className={`relative flex items-center gap-1.5 text-xs sm:text-sm font-semibold px-3 py-1.5 rounded-lg border transition-all cursor-pointer ${
              bookmarkCount > 0
                ? 'bg-rose-50 hover:bg-rose-100/90 text-rose-700 border-rose-200 shadow-xs'
                : 'bg-slate-100 hover:bg-slate-200/80 text-slate-700 border-slate-200/50'
            }`}
          >
            <Heart
              className={`w-4 h-4 transition-transform ${
                bookmarkCount > 0 ? 'fill-rose-500 text-rose-500' : 'text-slate-600'
              }`}
            />
          ...
          {currentStep !== 'hero' && (
            <button
              onClick={onReset}
              className="flex items-center gap-1.5 text-xs sm:text-sm font-medium text-slate-600 hover:text-slate-900 bg-slate-100 hover:bg-slate-200/80 px-3 py-1.5 rounded-lg transition-colors cursor-pointer"
            >
